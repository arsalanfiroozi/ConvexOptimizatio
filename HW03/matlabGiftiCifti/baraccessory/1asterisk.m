function [] = asterisk(xbar, ybar, xgroup, ygroup, str_show, bars, varargin)
    ...
    %
    % Xbar, Ybar: Which bars are you comparing? (1<=Xbar, Ybar<=number of bars in each group)
    % Xgroup, Ygroup: Which groups are you comparing? (1<=Xgroup, Ygroup<=number of groups in the bar) 
    %
    % String: The text you want to show
    %
    % Bar's Data: Data of barplot which it's size should be (n_Groups,n_Bars)
    %
    % Errors(Optional): matrix of errors which it's size should be (n_Groups,n_Bars)
    %
    % Distance1(Optional): The distance between end of bar column(or error line) and the bottom
    % of vertical lines
    %
    % Distance2(Optional): The distance between the closest point determined by Distance1 and
    % the horizontal line
    %

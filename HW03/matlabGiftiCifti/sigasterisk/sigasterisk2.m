function [] = sigasterisk2(xbar, ybar, xgroup, ygroup, str_show, bars, varargin)
    ...
    % Pre configuration
    XTicksOld = yticklabels;
    xticksold = yticks;
    height_v = xlim;
    height = height_v(2) - height_v(1);
%     xticks(1:ngroup);
%     set(gca, 'YTickLabel', 1:ngroup);
...
%     plot([P3_x P4_x], [P3_y P4_y], 'HandleVisibility','off','Color','black');
    ...
    %Text
    text(mean([P2_y, P3_y])+threshold(3) * sign(mean([P2_y, P3_y])) - threshold(3) * (sign(mean([P2_y, P3_y])) == -1), mean([P2_x, P3_x])+threshold(4)-0.06, str_show, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
    
%     xticks(xticksold);
%     set(gca,'YTickLabel', XTicksOld);

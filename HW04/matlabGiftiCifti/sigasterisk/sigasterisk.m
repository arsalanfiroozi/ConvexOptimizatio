function [] = sigasterisk(xbar, ybar, xgroup, ygroup, str_show, bars, varargin)
    ...
    threshold(1) = height * 4 / 125;
    threshold(2) = height * 2 / 125;
    threshold(3) = height * 0.5 / 125;
    threshold(4) = 0;
    ...
    %Text
    text(mean([P2_x, P3_x])+threshold(4), mean([P2_y, P3_y])+threshold(3) * sign(mean([P2_y, P3_y])) - threshold(3) * (sign(mean([P2_y, P3_y])) == -1), str_show, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle','FontSize',14);
